...
1. Set env vars in hosting platform:
   ...
   - `DIRECT_URL` — Neon **direct** (non-pooled) connection string (required for Prisma schema validation)
...
3. Apply schema migrations manually via Neon SQL Editor (advisory lock timeout prevents `prisma migrate deploy` on Neon free tier):
   ```sql
